...
3. **新建数据库**（项目规范名称：`Web-Programming-Course-Project`）：
  ```sql
  CREATE DATABASE `Web-Programming-Course-Project` DEFAULT CHARACTER SET utf8mb4;
  ```
4. **导入表结构**：
  - 使用 MySQL Workbench、Navicat 或命令行工具，运行项目中的 `database/BuildDatabase.sql` 文件。
  - 命令行示例（在项目根目录执行）：
    ```bash
    mysql -u root -p Web-Programming-Course-Project < database/BuildDatabase.sql
    ```
...
- 打开 `test.py` 文件，找到类似如下的数据库连接配置：
  ```python
  SQLALCHEMY_DATABASE_URL = "mysql+pymysql://root:123456@localhost:3306/Web-Programming-Course-Project?charset=utf8mb4"
  ```
