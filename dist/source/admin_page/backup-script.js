...
// 实际调用后端备份接口（与系统管理首页保持一致）
async function triggerBackendBackup() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            document.getElementById('backup-status-indicator')?.classList.add('online');
        } else {
            alert('创建备份失败，请稍后重试。');
        }
    } catch (error) {
        console.error('调用备份接口失败:', error);
...
function executeConfirmedRestore() {
    ...
    // 如果是“单个资源恢复”，调用后端 /{resource_type}/{id}/restore 接口
    if (restoreType === 'resource') {
        const resourceType = document.getElementById('resource-type').value;
        const resourceId = document.getElementById('resource-id').value;
        const apiBase = 'http://127.0.0.1:8000/api/v1';

        fetch(`${apiBase}/${resourceType}/${resourceId}/restore`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`
            }
            ...
            .catch(err => {
                console.error('资源恢复失败:', err);
                showMessage('资源恢复失败，请检查资源类型与ID是否正确', 'error');
                confirmBtn.innerHTML = originalText;
                confirmBtn.disabled = false;
            });
    } else {
